...
### 尾调用优化
函数调用会在内存中形成一个调用帧，保存内部变量和调用位置等等，然后js会在内存中记录一个调用栈，当调用函数A时，函数A入栈，如果在函数A的内部调用函数B，那么会在A的调用帧的上方形成一个B的调用帧，调用帧B入栈，等B调用结束后，将结果返回给A(可能没有返回)，此时B的调用帧就会消失，调用帧B出栈，然后等到A执行完了，调用帧A就会消失，此时调用栈pop调用帧A